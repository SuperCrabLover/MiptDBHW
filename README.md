...
- **Установка Redis**. Для установки `redis` я выбрал `docker`. Для экспериментов нам 
	...
	Конечно же можно было бы подключиться к `redis docker container` и использовать локальный питон, но тогда мне придется создать
	отдельную виртуальную среду для `redis-py`, поскольку существуеющие среды не хочется трогать. Зато в итоге мы получаем отдельный
