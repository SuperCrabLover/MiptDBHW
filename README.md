...
...
```
Вот он! Наш первый герой, слева направо: id, баланс и сколько с баланса в секунду улетает денег! Добавим
лаванды молодому:
```bash
...
Итого мне удалось написать очень удобные для использования скрипты, позволяющие скачать и запустить у себя в контейнере
целый `tarantool` кластер. При этом кластер шардируется самым правильным образом. Также скачивается и библотека для `TTL`.
...
Подключать `expirationd` будем тем же методом, что в примере на официальной страничке [git](https://github.com/tarantool/expirationd) даже 
имена у функции оставим теми же. Отдельное спасибо моему коллеге Дмитрию Яремусу, он подсказал, что устанавливать `expirationd` нужно еще и при
запуске кластера!

Пронаблюдать `ttl` можно следующим образом:

1. Запускаем класер и пишем `vshard.router.bootstrap()`, затем `insert_data()`. Вставляется три пользователя, которые не отслеживаются `ttl`:
```bash
bill:router-a-001> get(1)
---
...
```
Первая колонка это `id`, вторая -- баланс, третья -- сколько денег списывать в секунду, 
чертвертая -- флаг: отслеживать или нет, а пятое -- время последнего изменения любого поля.
Изменим количество списываемых денег первому пользователю и запустим процесс отслеживания,
прибавив к балансу ноль рублей:
```bash
...
...
```
Деньги списывются каждый `tick` библиотеки `expirationd`, но записанное время изменения 
сохраняет темп списывания постоянным. Спустя некоторое время денег не остается и пользователь
перестает быть отслеживаемым:
```bash
bill:router-a-001> get(1)
---
...
...
```
По поcледнему столбцу видно, что прошло около `10` секунд, как и должно быть.
