...
В `CAP` теореме `YDB` является `CP` базой данных.
...
## Query plan
Процесс выполнения запросов строится следующим образом: при отправке и распознавании команды языка `YQL` на
узлах начинается стадия выполнения запроса, она может содержать следующие операции:

- `TableFullScan` — Полное сканирование таблицы.
- `TableRangeScan` — Чтение строк таблицы по определённому диапазону значений первичного ключа.
- `TablePointLookup` — Чтение строк таблицы по конкретным значениям первичного ключа. 
- `Upsert` —  Перезапись строк таблицы по соответствующим значениям первичного ключа или добавление новых.
- `Delete` — Удаление строк из таблицы.
- `Join` — Объединение двух таблиц.
- `Filter` — Фильтрация строк.
- `Aggregate` — Группировка строк по значениям колонок с построением агрегатов.
- `Sort` — Сортировка строк.
- `TopSort` — Сортировка строк с применением лимита.
- `Top` — Взятие N элементов таким образом, что все они меньше или равны N+1 элемента, если бы вся последовательность была отсортирована.
- `Limit` — Ограничение на число строк.
- `Offset` — Смещение, позволяющее пропустить первые N элементов заданного набора строк.
- `Union` — Объединение результатов двух и более подзапросов в один набор.
- `Iterator` — Итератор по заданному набору строк.
- `PartitionByKey` — Партиционирование по ключу.

У одного запроса может быть сразу несколько стадий. Стадии классифицируются на `producer stage` — стадия, отправляющая данные
и `consumer stage` — стадия, принимающая данные.
Каждая стадия в свою очередь может состоять из нескольких "тасок".
Зависимости между стадиями по данным описываются с помощью 
понятия `connection`: от того, какой `connection` используется, зависит как именно данные будут передаваться между стадиями.

Типы `connection`:

- `UnionAll` — Объединяет результаты всех тасков `producer stage` и отдаёт их как единый результат в единственный таск `consumer stage`.
- `Merge` — `UnionAll`, но когда результаты отсортированы по заданному набору колонок; результат также отсортирован.
- `Broadcast` — Рассылает результат единственного таска `producer stage` на все таски `consumer stage`.
- `Map` — Реализует связи тасков стадий 1-к-1.
- `HashShuffle` — Рассылает результаты с тасков `producer stage` на таски `consumer stage` по некоторому правилу для заданных колонок.
...
Как видно план состоит из элементов, описанных выше.

## Transactions

По умолчанию транзакции в YDB выполняются в режиме `Serializable`, который предоставляет самый строгий уровень изоляции 
для пользовательских транзакций. В этом режиме гарантируется, что результат успешно выполненных параллельных 
транзакций эквивалентен определенному последовательному порядку их выполнения, при этом для успешных транзакций 
отсутствуют аномалии чтений.

Однако есть возможность выбрать режимы с пониженными гарантиями:
- `Online Read-Only` — каждое из чтений в транзакции читает последние на момент своего выполнения данные. 
    Консистентность полученных данных определяется настройкой `allow_inconsistent_reads`. Если настройка `false`, то
    отдельное чтение возвращает консистентный резльтат на текущий момент времени, иначе не гарантируется и это.
- `Stale Read-Only` — чтения данных в транзакции возвращают результаты с возможным отставанием от актуальных (доли секунды). 
    Данные в каждом отдельно взятом чтении консистентны, между разными чтениями консистентность данных не гарантируется.
- `Snapshot Read-Only` — все чтения транзакции производятся из снапшота базы данных, при этом все чтения данных консистентны. 
    Взятие снапшота происходит в момент старта транзакции, т.е. транзакция видит все изменения, закоммиченные до момента своего начала.


В YDB поддерживаются распределенные транзакции. Распределенные транзакции — это транзакции, которые затрагивают более 
одного шарда одной или нескольких таблиц.Они требуют больше ресурсов и выполняются дольше. 
В то время как точечные чтения и записи могут выполняться за время до 
10 мс в 99 перцентиле, распределенные транзакции, как правило, занимают от 20 до 500 мс.


[Источник](https://ydb.tech/docs/ru/concepts/transactions)
...
Для выполнения резервного копирования в файлы применяется команда `ydb tools dump`. 
Для выполнения восстановления из файловой резервной копии применяется команда `ydb tools restore`.
...
[Источник](https://ydb.tech/docs/ru/devops/manual/backup-and-recovery)

## Sharding

По умолчанию созданная таблица состоит из одного шарда. `YDB` поддерживает автоматическое шардирование таблицы по объему данных, 
т.е. при достижении определенного размера шард таблицы разделяется на два шарда.
Приемлемый размер для деления шарда таблицы — `2Gb`. С увеличением количества шардов пропускная 
способность загрузки данных увеличивается, но некоторое время с начала пропускная способность будет низкой.
...
## Data Mining, DW, OLTP & OLAP

- `OLAP` — В дизайн YDB заложена поддержка разных сценариев нагрузки, таких как OLTP и OLAP. 
    В текущей реализации поддержка аналитических запросов ограничена. Поэтому можно говорить, что в данный момент YDB — это OLTP-база данных.
- `Data Warehousing` — `YDB` имеет достаточное количество инструментов для этих целей. 
- `Data Mining` — `YDB` предназначена для высокой загрузки в бизнес задачах, поэтому применение в этих целях несет сомнительную пользу. 

## Security

`YDB` поддерживает аутентификацию по логину паролю. Кластер `YDB` имеет встроенные группы, предоставляющие заранее определенные наборы ролей:
- `ADMINS`
- `DATABASE-ADMINS`
- `ACCESS-ADMINS`
- `DDL-ADMINS`
...
Имеется поддержка аудитного лога. Данные на дисках можно шифровать.

[Источник](https://ydb.tech/docs/ru/security/access-management)

## Maintainers

Яндекс занимается активной поддержкой проекта: ведет свои [блоги](https://habr.com/ru/companies/ydb/articles/), 
занимается [тех. поддрежкой](https://ydb.tech/support) и пишет подробнейшую [документацию](https://ydb.tech/docs/en/).

Также в [официально туториале](https://ydb.tech/docs/en/quickstart) разработчики оставили указание по подъему аж двух демобаз:
одна состоит из одной простейшей таблицы `example`, а [вторая](https://ydb.tech/docs/en/dev/yql-tutorial/create_demo_tables) — для 
хранения сериалов — состоит из нескольких таблиц.

Получать свежие новости об обновлениях можно в любой удобной ~~и незапрещенной социальной~~ сети:
